...
# ---------------------------------------------------------------------------
# Background Task Worker — Asynchronous Delivery Pipeline
# ---------------------------------------------------------------------------
# Offloaded via FastAPI BackgroundTasks to avoid blocking the HTTP response.
# Pipeline:  Generate PDF Agreement → Generate LLM Email → Send via SMTP
#            → Save prediction record to loan_predictions table.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Batch Processing — AI Decision for All PENDING Applications
# ---------------------------------------------------------------------------
# Sets each pending loan to PROCESSING status to prevent double-picks,
# then enqueues process_auto_logic as a background task per loan.
# ---------------------------------------------------------------------------
...
def process_auto_logic(loan_id, applicant_name, applicant_email, loan_amount):
    """
    Shared AI processing logic used by both /api/process-auto and /api/process-batch.
    Loads the loan, runs XGBoost prediction, updates status, and triggers the
    background delivery pipeline (PDF + LLM email + SMTP).
    """
    try:
...
# ---------------------------------------------------------------------------
# Individual Auto-Processing — Single Loan AI Decision
# ---------------------------------------------------------------------------
# Called from the dashboard "AI Flow" button. Performs inline feature
# engineering, runs XGBoost prediction, updates DB, and offloads email
# + PDF delivery to a background task.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Manual Override — Human Approval / Rejection
# ---------------------------------------------------------------------------
# Allows admins to bypass the AI model and directly approve or reject a
# loan. Sets probability to 1.0 (approved) or 0.0 (rejected) and
# offloads email delivery to the background pipeline.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Document Download — Loan Agreement PDF Retrieval
# ---------------------------------------------------------------------------
# Returns the generated PDF loan agreement for a given loan_id.
# Documents are stored in the /documents directory, named by applicant.
# ---------------------------------------------------------------------------
